...
#      "/usr/bin/ng build --prod",
      ...
      "awsip=${aws_instance.web.public_ip}",
#      "~/.local/bin/aws sns publish --target-arn ${module.notify-slack.this_slack_topic_arn} --region ${var.aws_region} --message \"server provisioned at ip ${aws_instance.web.public_ip}\"",
      "~/.local/bin/aws sns publish --target-arn ${module.notify-slack.this_slack_topic_arn} --region ${var.aws_region} --message \"server provisioned for cody at $awsip\"",
    ]
  }
  tags = {
    AppName    = "TFDemoApp"
...
module "notify-slack" {
  source            = "app.terraform.io/jschulman-test/notify-slack/aws"
  version           = "4.14.0"
